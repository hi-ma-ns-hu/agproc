...
def configure_logging() -> None:
  ...
  Same JSONFormatter in every environment (dev, staging, prod) — dev sees
  exactly the same fields (trace_id, status, duration_ms, ...) that would
  reach dashboards in staging/prod, just printed to stdout instead of shipped.
  """
  ...
  logging.getLogger('sqlalchemy.engine').setLevel(logging.DEBUG if settings.IS_DEVELOPMENT else logging.WARNING)
...
__all__ = ['configure_logging', 'get_logger', 'bind_context', 'clear_context', 'init_tracing']
